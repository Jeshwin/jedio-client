<template>
  <div class="flex flex-row justify-start gap-4 h-auto pt-24 pb-4 border-b border-black text-left">
    ...
    <div class="grid grid-cols-2 w-full pt-14">
      ...
      <div v-if="project.userId" class="flex flex-row justify-start  w-full self-start -mt-2">
        <img class="w-10 h-10 mr-2 rounded-full ring-1 ring-black" src="@/assets/default_pfp.png">
        <a class="text-4xl text-gray-400 hover:text-purple-400 transition duration-300 font-quicksand font-semibold" :href="createrProfile" >By User #{{ project.userId }}</a>
      </div>
      <div v-else class="flex flex-row justify-start text-4xl w-full self-start -mt-2 font-quicksand text-gray-500 font-semibold">
        <img class="w-10 h-10 mr-2 rounded-full ring-1 ring-black" src="@/assets/default_pfp.png">
        By Anon
      </div>
