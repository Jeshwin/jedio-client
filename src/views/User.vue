<template>
	<div class="w-5/6 min-w-max mx-auto pt-24">
		<div class="grid grid-cols-1 gap-y-4 rounded-3xl bg-gray-200">
			<div class="h-64 w-full bg-gray-400 rounded-t-3xl border-b-2 border-black"
			></div>
			<div class="flex justify-between pl-16 -mt-28">
				<img
					class="w-48 h-48 rounded-full ring-2 ring-black bg-gray-50"
					:src="userAvatar"
				/>
				<router-link
					to="/user/edit"
					type="button"
					class="cursor-pointer w-auto h-12 px-4 py-2 mr-10 mt-28 text-2xl font-semibold rounded-full text-purple-400 ring-purple-400 ring-4 hover:text-green-600 hover:ring-green-600 transition duration-300"
					>Edit Profile</router-link
				>
			</div>
			<span
				class="flex justify-start pl-12 text-6xl text-black font-quicksand"
				>@{{ user.username }}</span
			>
			<div
				class="flex justify-start pl-14 text-2xl text-gray-800 font-quicksand"
			>
				<svg
			...
			<div
				class="flex justify-start pl-14 pb-2 text-xl text-gray-800 font-quicksand"
			>
				<svg
					xmlns="http://www.w3.org/2000/svg"
					viewBox="0 0 24 24"
			...
			<div class="grid grid-cols-3 gap-x-4 gap-y-8 mt-4 mx-12 mb-12">
				<div
					...
					:key="project.id"
					class="bg-white rounded-2xl relative h-var w-var"
				>
					<a
						class="absolute bottom-0 right-0 z-40 h-10 w-10 m-4 p-4 cursor-pointer rounded-full text-black bg-yellow-400"
						:href="`http://localhost:8080/update/project/${project.id}`"
					>
						<svg
							class="centerEdit"
							xmlns="http://www.w3.org/2000/svg"
				...
				<router-link
					to="/create/project"
					class="bg-red-400 rounded-3xl relative h-var w-var flex hover:bg-green-400 transition duration-500 cursor-pointer"
				>
					<svg
						xmlns="http://www.w3.org/2000/svg"
						class="place-self-center h-40 mx-auto fill-current stroke-current text-white"
						viewBox="0 0 16 16"
						width="720"
						height="720"
					>
...
export default {
	...
	async created() {
		...
		const Projsres = await axios
			.get(`http://localhost:3000/project/user/${this.user.id}`)
