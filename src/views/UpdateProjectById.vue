<template>
  ...
  <form enctype="application/x-www-form-urlencoded" action="http://localhost:3000/update/project" method="post">
    <input class="hidden" type="hidden" name="oldtitle" id="oldtitle" :value="titleById">
    <div class="flex flex-row h-14 my-3 border-b border-purple-200">
      <label for="title">New Title:</label>
      <input class="w-56 overflow-x-auto" type="text" name="title" id="title" v-model="title">
    </div>
    <div class="flex flex-row h-14 my-3 border-b border-purple-200">
      <label for="category">New Category:</label>
      <input class="w-36 overflow-x-auto" type="text" name="category" id="category" v-model="category">
    </div>
...
export default {
  ...
  async created () {
    axios.get(`http://localhost:3000/project/${this.$route.params.id}`).then((res) => {
