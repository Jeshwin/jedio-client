  ...
  <div class="w-1/2 min-w-max mx-auto pt-28 pb-12">
    <form enctype="application/x-www-form-urlencoded" action="javascript:void(0);" @submit="registerUser" method="post">
      ...
      <div class="flex flex-row h-14 my-3">
        ...
        <input class="w-full overflow-x-auto" type="email" name="email" id="email" v-model="email">
      </div>
      <div class="flex flex-row h-14 my-3">
      ...
      <div class="flex flex-row h-14 my-3">
        <label for="isAdmin" class="flex items-center cursor-pointer">
          ...
          <div class="relative">
            <!-- input -->
            <input name="isAdmin" id="isAdmin" type="checkbox" class="sr-only" v-model="isAdmin" />
            <!-- line -->
            <div class="block line w-14 h-8 bg-purple-300 rounded-full shadow-inner transition"></div>
            <!-- dot -->
            <div class="dot absolute w-6 h-6 bg-purple-50 rounded-full left-1 top-1 transition"></div>
          </div>
        </label>
      </div>
